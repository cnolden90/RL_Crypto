    ...
    def preprocess_data(self):
        if(self.existingData):
            ...
            for coin in self.coins:
                ...
                """ not scaling time & FGI """
                df_not_scale = data[['dayOfWeek_sin','dayOfWeek_cos','dayInYear_sin','dayInYear_cos','timeOfDay_sin','timeOfDay_cos','monthInYear_sin','monthInYear_cos', 'classification_index']].copy()
                data = data.drop(['dayOfWeek_sin','dayOfWeek_cos','dayInYear_sin','dayInYear_cos','timeOfDay_sin','timeOfDay_cos','monthInYear_sin','monthInYear_cos', 'classification_index'], axis=1)
                
                """ make data stationary: OHCLV &  MC""" 
                if(self.taIndicator):
                    make_stationary = ['open','high','close','low','volume','market_cap', 'returns', 'ret_2','ret_5','ret_10','ret_21','rsi']
                else:
                ...
                data = pd.concat([data, df_not_scale], axis=1)
